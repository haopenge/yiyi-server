...
public class StrategyAbstract implements Strategy {
    ...
    Server getRandom(List<Server> devlist) {
        ...
        if (devsize == 1) {
            try {
                this.logger.info(String.format("服务:%s策略:%s命中一个Server%s", new Object[]{((Server) devlist.get(0)).getMetaInfo().getAppName(), this.name, ((Server) devlist.get(0)).getHost()}));
            } catch (Exception e) {
        ...
        if (devsize > 1) {
            ...
            try {
                this.logger.info(String.format("服务:%s策略%s命中%d个Server,choose%d", new Object[]{((Server) devlist.get(0)).getMetaInfo().getAppName(), this.name, Integer.valueOf(devsize), Integer.valueOf(number)}).concat(((Server) devlist.get(number)).getHost()));
            } catch (Exception e) {
                this.logger.error("getRandom", e);
            }
