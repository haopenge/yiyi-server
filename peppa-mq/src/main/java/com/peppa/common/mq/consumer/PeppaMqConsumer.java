package com.peppa.common.mq.consumer;

import brave.Span;
import com.alibaba.fastjson.JSONObject;
import com.peppa.common.mq.acc.Idempotent;
import com.peppa.common.mq.base.ConsumeMode;
import com.peppa.common.mq.base.MqClient;
import com.peppa.common.mq.base.PeppaInfoLevelException;
import com.peppa.common.mq.base.PeppaMqException;
import com.peppa.common.mq.span.MqSpan;
...
public class PeppaMqConsumer implements Closeable {
    private static final Logger log = LoggerFactory.getLogger(PeppaMqConsumer.class);
    ...
    private String idempotentFactoryClass = "com.peppa.common.mq.acc.impl.IdempotentFactory";

    public static void setIdempotent(Idempotent idempotent) {
        PeppaMqConsumer.idempotent = idempotent;
    }
    ...
    public void initConsumer() {
        ...
        try {
            Class.forName("com.peppa.common.handler.mqstarter.MqConsumerStarter");
            log.warn("MQ消费者初始化，检查到com.peppa.common.handler.mqstarter.MqConsumerStarter！");
        } catch (ClassNotFoundException e) {
            this.isLazy = false;
        ...
        if (orderly) {
            consumer.registerMessageListener(new MessageListenerOrderly() {
                public ConsumeOrderlyStatus consumeMessage(List<MessageExt> list, ConsumeOrderlyContext consumeOrderlyContext) {
                    ...
                    Span s = null;
                    try {
                        if (PeppaMqConsumer.this.isIdempotent) {
                            try {
                                PeppaMqConsumer.idempotent.begin(list, realConsumerName);
                            } catch (PeppaMqException pe) {
                                PeppaMqConsumer.log.warn("消息重复, e", (Throwable) pe);
                                return ConsumeOrderlyStatus.SUCCESS;
                            } catch (Exception e) {
                                PeppaMqConsumer.log.error("幂等处理未知错误,请联系技术处理，同时幂等失效，e", e);
                            }
                        }
                        if (isList) {
                            if (PeppaMqConsumer.this.isGray) {

                                List<MessageExt> listchecked = PeppaMqConsumer.this.getCheckDiscardList(list, consumer, topic, publicGroupName);
                            ...
                            for (MessageExt messageExt : list) {
                                if (!PeppaMqConsumer.this.checkDiscard(messageExt, consumer, topic, publicGroupName)) {
                        ...
                        PeppaMqConsumer.log.info("重复消费，返回消费成功, e", (Throwable) pe);
                        span.error(s, (Throwable) pe);
                        return ConsumeOrderlyStatus.SUCCESS;
                    } catch (PeppaInfoLevelException pile) {
                        PeppaMqConsumer.log.info("pile push处理顺序消息异常  e", (Throwable) pile);

                        try {
                            list.forEach(me -> PeppaMqConsumer.log.info("pile push order error:{}", JSONObject.toJSONString(me)));

                        } catch (Exception e1) {
                            PeppaMqConsumer.log.info("pile 打印日志出错，e", (Throwable) pile);
                        }
                        try {
                            if (PeppaMqConsumer.this.isIdempotent) {
                                PeppaMqConsumer.idempotent.failed(list, realConsumerName);
                            }
                        } catch (Exception e1) {
                            PeppaMqConsumer.log.info("pile 幂等异常：e", (Throwable) pile);
                        }
                        return ConsumeOrderlyStatus.SUSPEND_CURRENT_QUEUE_A_MOMENT;
                    } catch (Exception e) {
                        PeppaMqConsumer.log.error("push处理顺序消息异常  e", e);
                        span.error(s, e);
                        try {
                            list.forEach(me -> PeppaMqConsumer.log.info("push order error:{}", JSONObject.toJSONString(me)));

                        } catch (Exception e1) {
                            PeppaMqConsumer.log.error("打印日志出错，e", e);
                        }
                        try {
                            if (PeppaMqConsumer.this.isIdempotent) {
                                PeppaMqConsumer.idempotent.failed(list, realConsumerName);
                            ...
                            PeppaMqConsumer.log.error("幂等异常：e", e);
                        }
                        return ConsumeOrderlyStatus.SUSPEND_CURRENT_QUEUE_A_MOMENT;
                    }
                }
            ...
            consumer.registerMessageListener(new MessageListenerConcurrently() {
                public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> list, ConsumeConcurrentlyContext consumeConcurrentlyContext) {
                    ...
                    try {
                        ...
                        PeppaMqConsumer.log.info("pile push处理消息异常,延时重新消费 e", (Throwable) pile);
                        ...
                        PeppaMqConsumer.log.error("push处理消息异常,延时重新消费 e ", e);
                        span.error(s, e);
        ...
        mqPullConsumerScheduleService.registerPullTaskCallback(topic, new PullTaskCallback() {
            public void doPullTask(MessageQueue mq, PullTaskContext pullTaskContext) {
                ...
                try {
                        ...
                        case FOUND:
                            ...
                            if (isList) {
                                try {
                                    ...
                                    PeppaMqConsumer.log.info("pile pull list 业务处理失败,e", (Throwable) pile);

                                    boolean b = PeppaMqConsumer.this.pullRetry(method, mqConsumerCallback, msgFoundList);
                                    if (!b) {
                                        msgFoundList.forEach(me -> PeppaMqConsumer.log.info("pile pull list error:{}", JSONObject.toJSONString(me)));

                                    }

                                } catch (Exception e) {
                                    PeppaMqConsumer.log.error("pull list 业务处理失败,e", e);

                                    boolean b = PeppaMqConsumer.this.pullRetry(method, mqConsumerCallback, msgFoundList);
                                    if (!b) {
                                        msgFoundList.forEach(me -> PeppaMqConsumer.log.info("pull list error:{}", JSONObject.toJSONString(me)));
                                    }
                            ...
                            for (MessageExt messageExt : msgFoundList) {
                                try {
                                    ...
                                    PeppaMqConsumer.log.info("pile pull messageExt 业务处理失败,e", (Throwable) pile);

                                    boolean b = PeppaMqConsumer.this.pullRetry(method, mqConsumerCallback, messageExt);
                                    if (!b) {
                                        PeppaMqConsumer.log.info("pile pull object error:{}", JSONObject.toJSONString(messageExt));
                                    }
                                } catch (Exception e) {
                                    PeppaMqConsumer.log.error("pull messageExt 业务处理失败,e", e);
                                    span.error(s, e);
                                    boolean b = PeppaMqConsumer.this.pullRetry(method, mqConsumerCallback, messageExt);
                                    if (!b) {
                                        PeppaMqConsumer.log.info("pull object error:{}", JSONObject.toJSONString(messageExt));
                                    }
                                }
                            }
                            break;
                    ...
                    PeppaMqConsumer.log.error("topic:{} consume message error, e", topic, e);
                    e.printStackTrace();
                }
            }
        });
    ...
    private boolean checkParameter(Parameter[] parameters) {
        ...
        if (parameters != null && parameters.length > 0) {
            ...
            if ("java.util.List".equals(name)) {
                ...
                throw new PeppaMqException("consumer 参数类型错误");
            }
        } else {
            throw new PeppaMqException("consumer 方法没指定参数");
        }
        return isList;
    }
            ...
            if (annotation != null) {
                ...
                if (consumeMode.equals(ConsumeMode.PUSH)) {
                    workThread = new Thread(new Runnable() {
                        public void run() {
                            try {
                                PeppaMqConsumer.log.info("pushConsumer start, topic:{},tag:{}", topic, tag);
                                PeppaMqConsumer.this.createDefaultMQPushConsumer(messageModel, mqConsumerCallback, method, topic, tag, orderly, isList, groupName, minThread, maxThread);
                                PeppaMqConsumer.log.info("pushConsumer start, topic:{},tag:{},status:{}", topic, tag, "success");
                            } catch (Exception e) {
                                PeppaMqConsumer.log.error("pushConsumer start failed, topic:{},tag:{}，e:" + e, topic, tag);
                            }
                        }
                    }, threadName);
                    workThread.start();
                ...
                if (consumeMode.equals(ConsumeMode.PULL)) {
                    workThread = new Thread(new Runnable() {
                        public void run() {
                            try {
                                PeppaMqConsumer.log.info("pullConsumer start, topic:{},tag:{}", topic, tag);
                                PeppaMqConsumer.this.createMQPullConsumerScheduleService(messageModel, mqConsumerCallback, method, topic, tag, isList, groupName, batchSize);
                                PeppaMqConsumer.log.info("pullConsumer start failed, topic:{},tag:{},status:{}", topic, tag, "success");
                            } catch (Exception e) {
                                PeppaMqConsumer.log.error("pullConsumer start failed, topic:{},tag:{}", topic, tag);
                            }
                        }
                    }, threadName);
                    workThread.start();
    ...
    private String getTopicGroupRulePodenv(String topicName, String groupName) {
        try {
            Class<?> threadlocalClazz = Class.forName("com.peppa.common.grayconfig.apolloconfig.MqGrayApolloConfig");
    ...
    private void addHeaderToThreadLocal(String podenv) {
        ...
        try {
            Class<?> threadlocalClazz = Class.forName("com.peppa.common.grayconfig.ThreadAttributes");
            Method method = threadlocalClazz.getMethod("setThreadAttribute", String.class, Object.class);
            method.invoke((Object) null, "huohua-podenv", podenv);
        } catch (Exception e) {
            log.warn("mq消费者{}没有找到，没有引用架构组件！微服务环境必须引用！", "com.peppa.common.grayconfig.ThreadAttributes");
        }
    }
}